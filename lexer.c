...
static token_t make_tok(tok_type_t t, const char *start, size_t len, int line, int col)
{
    token_t tok;
    tok.type  = t;
    tok.start = start;
...
static token_t lex_string(parser_t *p)
{
    ...
    set_error(p, sline, scol, "unterminated string starting at %d:%d", sline, scol);
...
static token_t lex_number(parser_t *p)
{
    ...
    if (cur_ch(p) == '0') {
        ...
        while (isdigit((unsigned char)cur_ch(p))) advance(p);
    }
    ...
    if (cur_ch(p) == '.') {
        ...
        if (!isdigit((unsigned char)cur_ch(p))) {
            set_error(p, sline, scol,
                      "expected digit after decimal point at %d:%d", sline, scol);
            return make_tok(TOK_ERROR, NULL, 0, sline, scol);
        }
        while (isdigit((unsigned char)cur_ch(p))) advance(p);
    }
    ...
    if (cur_ch(p) == 'e' || cur_ch(p) == 'E') {
        advance(p);
        if (cur_ch(p) == '+' || cur_ch(p) == '-') advance(p);
        if (!isdigit((unsigned char)cur_ch(p))) {
            set_error(p, sline, scol,
                      "expected digit in exponent at %d:%d", sline, scol);
            return make_tok(TOK_ERROR, NULL, 0, sline, scol);
        }
        while (isdigit((unsigned char)cur_ch(p))) advance(p);
    }
...
static token_t lex_keyword(parser_t *p, const char *kw, tok_type_t t)
{
    ...
    if (strncmp(p->src + p->pos, kw, klen) == 0
        ...
        const char *start = p->src + p->pos;
        for (size_t i = 0; i < klen; i++) advance(p);
...
void lexer_init(parser_t *p, const char *src, json_error_t *err)
{
    ...
    p->err   = err;
    p->depth = 0;
    if (err) { err->message[0] = '\0'; err->line = 0; err->col = 0; }
    /* prime the lookahead */
    p->cur = lexer_next(p);
}
...
token_t lexer_next(parser_t *p)
{
    ...
    if (c == '\0') return make_tok(TOK_EOF, NULL, 0, sline, scol);

    switch (c) {
        case '{': advance(p); return make_tok(TOK_LBRACE,   NULL, 0, sline, scol);
        case '}': advance(p); return make_tok(TOK_RBRACE,   NULL, 0, sline, scol);
        case '[': advance(p); return make_tok(TOK_LBRACKET, NULL, 0, sline, scol);
        case ']': advance(p); return make_tok(TOK_RBRACKET, NULL, 0, sline, scol);
        case ':': advance(p); return make_tok(TOK_COLON,    NULL, 0, sline, scol);
        case ',': advance(p); return make_tok(TOK_COMMA,    NULL, 0, sline, scol);
        case '"': return lex_string(p);
        case 't': return lex_keyword(p, "true",  TOK_TRUE);
        case 'f': return lex_keyword(p, "false", TOK_FALSE);
        case 'n': return lex_keyword(p, "null",  TOK_NULL);
        default:
            if (c == '-' || isdigit((unsigned char)c))
                return lex_number(p);
            set_error(p, sline, scol,
                      "unexpected character '%c' at %d:%d", c, sline, scol);
            return make_tok(TOK_ERROR, NULL, 0, sline, scol);
    }
}
...
const char *tok_type_name(tok_type_t t)
{
    ...
    if ((int)t < 0 || (int)t > TOK_ERROR) return "?";
    return names[(int)t];
}
