...
char *unescape_string(parser_t *p, const char *raw, size_t len)
{
    ...
    for (size_t i = 0; i < len;)
    {
        ...
        case 'u': {
            ...
            {
                set_error(p, 0, 0, "invalid hex in \\uXXXX escape: %.4s",
                          raw + i);
                free(out);
                return NULL;
            }
            ...
            if (hi >= 0xD800 && hi <= 0xDBFF)
            {
                ...
                {
                    set_error(p, 0, 0,
                              "high surrogate U+%04X not followed by \\uXXXX",
                              hi);
                    free(out);
                    return NULL;
                }
                ...
                {
                    set_error(p, 0, 0, "expected low surrogate, got U+%04X",
                              lo);
                    free(out);
                    return NULL;
                }
            ...
            {
                set_error(p, 0, 0, "unexpected low surrogate U+%04X", hi);
                free(out);
                return NULL;
            }
        ...
        default:
            set_error(p, 0, 0, "invalid escape sequence '\\%c'", raw[i]);
            free(out);
            return NULL;
        }
    }
