...
static int serialize(buf_t *b, const json_value_t *v, int indent, int depth)
{
    ...
    case JSON_OBJECT: {
        ...
        while (pair)
        {
            ...
            if (write_escaped_string(b, pair->key) < 0)
                return -1;
            if (buf_push(b, ':') < 0)
                return -1;
            if (indent >= 0 && buf_push(b, ' ') < 0)
                return -1;
            if (serialize(b, pair->value, indent, depth + 1) < 0)
                return -1;
            pair = pair->next;
        }
