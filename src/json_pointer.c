                               ...
                               json_pointer_error_t *err)
{
    ...
    for (;;)
    {
        ...
        if (json_is_array(cur))
        {
            ...
            {
                ptr_err(err, "array index %zu out of bounds (len=%zu)", idx,
                        cur->v.array.count);
                free(tok);
                return NULL;
            }
                     ...
                     json_value_t *value, json_pointer_error_t *err)
{
    ...
    if (!prev)
    {
        ptr_err(err, "empty pointer after parsing");
        return 0;
    }
                        ...
                        json_pointer_error_t *err)
{
    ...
    if (*pointer == '\0')
    {
        ptr_err(err, "cannot remove root");
        return 0;
    }
    if (*pointer != '/')
    {
        ptr_err(err, "pointer must start with '/'");
        return 0;
    }
