...
static int fail(json_schema_error_t *err, const char *path,
                const char *fmt, ...)
{
    if (!err) return 0;
    va_list ap;
...
{
    if (strcmp(t, "null")    == 0) return json_is_null(val);
    if (strcmp(t, "boolean") == 0) return json_is_bool(val);
    if (strcmp(t, "number")  == 0) return json_is_number(val);
    if (strcmp(t, "integer") == 0)
        return json_is_number(val) && val->v.number == floor(val->v.number);
    if (strcmp(t, "string")  == 0) return json_is_string(val);
    if (strcmp(t, "array")   == 0) return json_is_array(val);
    if (strcmp(t, "object")  == 0) return json_is_object(val);
    return 1;
}
...
{
    if (!json_is_object(schema)) return NULL;
...
static int validate(const json_value_t *schema, const json_value_t *val,
                    json_schema_error_t *err, char *path)
{
    if (!json_is_object(schema)) return 1;
    ...
    if ((node = sget(schema, "enum")) && json_is_array(node)) {
        ...
        for (size_t i = 0; i < node->v.array.count; i++)
            if (values_equal(val, node->v.array.items[i])) { found = 1; break; }
        if (!found) return fail(err, path, "value not in enum");
    }
    ...
    if (json_is_string(val)) {
        ...
        if ((node = sget(schema, "minLength")) && json_is_number(node) &&
            ...
            return fail(err, path, "string length %zu < minLength %.0f",
                        slen, node->v.number);
        if ((node = sget(schema, "maxLength")) && json_is_number(node) &&
            slen > (size_t)node->v.number)
            return fail(err, path, "string length %zu > maxLength %.0f",
                        slen, node->v.number);
    }
    ...
    if (json_is_number(val)) {
        if ((node = sget(schema, "minimum")) && json_is_number(node) &&
            ...
            return fail(err, path, "%.17g < minimum %.17g",
                        val->v.number, node->v.number);
        if ((node = sget(schema, "maximum")) && json_is_number(node) &&
            val->v.number > node->v.number)
            return fail(err, path, "%.17g > maximum %.17g",
                        val->v.number, node->v.number);
    }
    ...
    if (json_is_array(val)) {
        ...
        if ((node = sget(schema, "minItems")) && json_is_number(node) &&
            ...
            return fail(err, path, "array length %zu < minItems %.0f",
                        count, node->v.number);
        if ((node = sget(schema, "maxItems")) && json_is_number(node) &&
            count > (size_t)node->v.number)
            return fail(err, path, "array length %zu > maxItems %.0f",
                        count, node->v.number);
    ...
    if (json_is_object(val)) {
        ...
        if (req && json_is_array(req)) {
            for (size_t i = 0; i < req->v.array.count; i++) {
                ...
                for (const json_pair_t *p = val->v.object.head; p; p = p->next)
                    if (strcmp(p->key, rk->v.string) == 0) { found = 1; break; }
                if (!found)
        ...
        if (props && json_is_object(props)) {
            for (const json_pair_t *vp = val->v.object.head; vp; vp = vp->next) {
                ...
                for (const json_pair_t *sp = props->v.object.head; sp; sp = sp->next)
                    if (strcmp(sp->key, vp->key) == 0) { ps = sp->value; break; }
                if (ps) {
                    char cp[512];
                    snprintf(cp, sizeof(cp), "%s/%s", path, vp->key);
                    if (!validate(ps, vp->value, err, cp)) return 0;
                }
            }
        }
        ...
        if (addl && json_is_bool(addl) && !addl->v.boolean && props) {
            for (const json_pair_t *vp = val->v.object.head; vp; vp = vp->next) {
                int known = 0;
                for (const json_pair_t *sp = props->v.object.head; sp; sp = sp->next)
                    if (strcmp(sp->key, vp->key) == 0) { known = 1; break; }
                if (!known)
                    return fail(err, path,
                                "additional property '%s' not allowed", vp->key);
            }
        }
    }
...
{
    if (err) { err->path[0] = '\0'; err->message[0] = '\0'; }
    char root_path[2] = "";
