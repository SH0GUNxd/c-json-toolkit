...
json_value_t *json_clone(const json_value_t *val)
{
    ...
    switch (val->type) {
        ...
        case JSON_STRING:
            ...
            if (!out->v.string) { free(out); return NULL; }
            strcpy(out->v.string, val->v.string);
            break;

        case JSON_ARRAY: {
            size_t n = val->v.array.count;
            out->v.array.items = (json_value_t **)calloc(n ? n : 1,
                                                         sizeof(json_value_t *));
            if (!out->v.array.items) { free(out); return NULL; }
            out->v.array.count = n;
        ...
        case JSON_OBJECT: {
            ...
            for (const json_pair_t *p = val->v.object.head; p; p = p->next) {
                ...
                if (!np) { json_free(out); return NULL; }
                np->key = (char *)malloc(strlen(p->key) + 1);
                if (!np->key) { free(np); json_free(out); return NULL; }
                strcpy(np->key, p->key);
                np->value = json_clone(p->value);
                if (!np->value) { free(np->key); free(np); json_free(out); return NULL; }
                if (!out->v.object.head) out->v.object.head = np;
                else                     tail->next = np;
                tail = np;
                out->v.object.count++;
            }
...
static int values_equal(const json_value_t *a, const json_value_t *b)
{
    ...
    switch (a->type) {
        ...
        case JSON_BOOL:   return a->v.boolean == b->v.boolean;
        case JSON_NUMBER: return a->v.number  == b->v.number;
        case JSON_STRING: return strcmp(a->v.string, b->v.string) == 0;
        case JSON_ARRAY:
            if (a->v.array.count != b->v.array.count) return 0;
            for (size_t i = 0; i < a->v.array.count; i++)
                if (!values_equal(a->v.array.items[i], b->v.array.items[i]))
                    return 0;
            return 1;
        case JSON_OBJECT: {
            if (a->v.object.count != b->v.object.count) return 0;
            for (const json_pair_t *p = a->v.object.head; p; p = p->next) {
                ...
                for (const json_pair_t *q = b->v.object.head; q; q = q->next)
                    if (strcmp(p->key, q->key) == 0 &&
                        values_equal(p->value, q->value)) { found = 1; break; }
                if (!found) return 0;
            }
            return 1;
        }
...
static int apply_op(json_value_t       *doc,
                    const json_value_t *op,
                    int                 idx,
                    json_patch_error_t *err)
{
    ...
    if (strcmp(op_str, "add") == 0) {
        ...
        if (!val) { patch_err(err, idx, "op 'add': missing 'value'"); return 0; }
        json_value_t *clone = json_clone(val);
        if (!clone) { patch_err(err, idx, "op 'add': out of memory"); return 0; }
        if (*path_str == '\0') {
            patch_err(err, idx, "op 'add': cannot add to root with empty path");
            json_free(clone); return 0;
        }
    ...
    if (strcmp(op_str, "replace") == 0) {
        const json_value_t *val = obj_get(op, "value");
        if (!val) { patch_err(err, idx, "op 'replace': missing 'value'"); return 0; }
        /* target must exist */
        if (!json_pointer_get(doc, path_str, &perr)) {
            patch_err(err, idx, "op 'replace': target not found: %s", perr.message);
            return 0;
        }
        json_value_t *clone = json_clone(val);
        if (!clone) { patch_err(err, idx, "op 'replace': out of memory"); return 0; }
        if (!json_pointer_set(doc, path_str, clone, &perr)) {
    ...
    if (strcmp(op_str, "move") == 0) {
        ...
        if (!from_node || !json_is_string(from_node)) {
            patch_err(err, idx, "op 'move': missing 'from'"); return 0;
        }
        const char *from_str = from_node->v.string;
        json_value_t *src = json_pointer_get(doc, from_str, &perr);
        if (!src) {
            patch_err(err, idx, "op 'move': source not found: %s", perr.message);
            return 0;
        }
        json_value_t *clone = json_clone(src);
        if (!clone) { patch_err(err, idx, "op 'move': out of memory"); return 0; }
    ...
    if (strcmp(op_str, "copy") == 0) {
        const json_value_t *from_node = obj_get(op, "from");
        if (!from_node || !json_is_string(from_node)) {
            patch_err(err, idx, "op 'copy': missing 'from'"); return 0;
        }
        json_value_t *src = json_pointer_get(doc, from_node->v.string, &perr);
        if (!src) {
            patch_err(err, idx, "op 'copy': source not found: %s", perr.message);
            return 0;
        }
        json_value_t *clone = json_clone(src);
        if (!clone) { patch_err(err, idx, "op 'copy': out of memory"); return 0; }
        if (!json_pointer_set(doc, path_str, clone, &perr)) {
    ...
    if (strcmp(op_str, "test") == 0) {
        const json_value_t *val = obj_get(op, "value");
        if (!val) { patch_err(err, idx, "op 'test': missing 'value'"); return 0; }
...
json_value_t *json_patch_apply(const json_value_t *doc,
                               ...
                               json_patch_error_t *err)
{
    if (err) { err->message[0] = '\0'; err->op_index = -1; }
    ...
    if (!working) { patch_err(err, -1, "out of memory"); return NULL; }
