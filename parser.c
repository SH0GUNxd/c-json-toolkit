...
static token_t peek(const parser_t *p) { return p->cur; }
...
static int expect(parser_t *p, tok_type_t t)
{
    if (p->cur.type != t) {
        set_error(p, p->cur.line, p->cur.col,
                  ...
                  tok_type_name(t), tok_type_name(p->cur.type),
...
static json_value_t *parse_string(parser_t *p)
{
    ...
    if (!v) { free(s); set_error(p, 0, 0, "out of memory"); return NULL; }
    v->v.string = s;
    return v;
}
...
static json_value_t *parse_number(parser_t *p)
{
    ...
    if (end != buf + len) {
        set_error(p, tok.line, tok.col,
                  "malformed number '%s' at %d:%d", buf, tok.line, tok.col);
        return NULL;
    }

    json_value_t *v = alloc_value(JSON_NUMBER);
    if (!v) { set_error(p, 0, 0, "out of memory"); return NULL; }
    v->v.number = d;
    return v;
}
...
static json_value_t *parse_array(parser_t *p)
{
    ...
    if (!arr) { set_error(p, 0, 0, "out of memory"); return NULL; }

    if (peek(p).type == TOK_RBRACKET) { consume(p); return arr; }
    ...
    for (;;) {
        ...
        if (count == cap) {
            cap *= 2;
            json_value_t **tmp = (json_value_t **)realloc(items, cap * sizeof(*items));
            if (!tmp) {
                json_free(elem);
                set_error(p, 0, 0, "out of memory");
                goto err;
        ...
        if (peek(p).type == TOK_RBRACKET) break;
        if (!expect(p, TOK_COMMA)) goto err;

        /* trailing comma check */
        if (peek(p).type == TOK_RBRACKET) {
            set_error(p, p->cur.line, p->cur.col,
                      "trailing comma in array at %d:%d",
                      p->cur.line, p->cur.col);
            goto err;
        }
    }
...
static json_value_t *parse_object(parser_t *p)
{
    ...
    if (!obj) { set_error(p, 0, 0, "out of memory"); return NULL; }

    if (peek(p).type == TOK_RBRACE) { consume(p); return obj; }
    ...
    for (;;) {
        ...
        if (peek(p).type != TOK_STRING) {
            set_error(p, p->cur.line, p->cur.col,
                      "expected string key in object at %d:%d",
                      p->cur.line, p->cur.col);
            goto err;
        }
        ...
        if (!expect(p, TOK_COLON)) { free(key); goto err; }
        ...
        if (peek(p).type == TOK_RBRACE) break;
        if (!expect(p, TOK_COMMA)) goto err;

        /* trailing comma check */
        if (peek(p).type == TOK_RBRACE) {
            set_error(p, p->cur.line, p->cur.col,
                      "trailing comma in object at %d:%d",
                      p->cur.line, p->cur.col);
            goto err;
        }
    }
...
json_value_t *parse_value(parser_t *p)
{
    if (p->depth > MAX_DEPTH) {
        set_error(p, p->cur.line, p->cur.col,
                  "maximum nesting depth %d exceeded at %d:%d",
                  MAX_DEPTH, p->cur.line, p->cur.col);
        return NULL;
    }
    p->depth++;
    ...
    switch (t) {
        case TOK_NULL: {
            ...
            if (!v) set_error(p, tok.line, tok.col, "out of memory");
            break;
        }
        case TOK_TRUE: {
            token_t tok = consume(p);
        ...
        case TOK_NUMBER:  v = parse_number(p); break;
        case TOK_STRING:  v = parse_string(p); break;
        case TOK_LBRACKET: v = parse_array(p);  break;
        case TOK_LBRACE:   v = parse_object(p); break;
        default:
            set_error(p, p->cur.line, p->cur.col,
